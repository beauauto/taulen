    ...
    const loadExistingData = async () => {
      ...
      try {
        ...
        if (appData?.coBorrower || appData?.coBorrowerId) {
          ...
          if (coBorrower.currentAddress || coBorrower.currentResidence?.fullAddress) {
            const addr = coBorrower.currentAddress || coBorrower.currentResidence?.fullAddress || ''
          ...
          const loadedData = {
            ...
            suffix: coBorrower.suffix || '',
            ssn: coBorrower.ssn || coBorrower.taxpayerIdentifierValue || '',
            dateOfBirth: coBorrower.dateOfBirth || coBorrower.birthDate || '',
            
            // Contact Information
            email: coBorrower.email || '',
            homePhone: coBorrower.homePhone ? formatPhoneNumber(coBorrower.homePhone) : '',
            mobilePhone: coBorrower.phone && coBorrower.phoneType === 'MOBILE' ? formatPhoneNumber(coBorrower.phone) : '',
            workPhone: coBorrower.workPhone ? formatPhoneNumber(coBorrower.workPhone) : '',
            workPhoneExt: coBorrower.workPhoneExt || coBorrower.workPhoneExtension || '',
            preferredContactMethod: coBorrower.preferredContactMethod || 'EMAIL',
            
            // Current Address
            currentAddress: coBorrower.currentAddress || coBorrower.currentResidence?.fullAddress || '',
            ...
            alienRegistrationNumber: coBorrower.alienRegistrationNumber || '',
            
            // Marital Status
            maritalStatus: coBorrower.maritalStatus || '',
            
            // Dependents
            dependentCount: coBorrower.dependentCount?.toString() || '',
            dependentAges: coBorrower.dependentAges || '',
            
            // Military Status
            isVeteran: coBorrower.isVeteran || coBorrower.militaryServiceStatus || false,
            
            // Co-borrower specific
            liveTogether: coBorrower.liveTogether !== undefined ? coBorrower.liveTogether : true,
          }
  ...
  if (isLoading) {
    return (
      <Form1003Layout
        ...
        title="Edit Co-Borrower Information"
        onBack={handleBack}
      >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-[#e6e6e6] rounded-lg p-3 md:p-4 text-left text-sm md:text-base text-[#334D5C] relative mb-2.5 md:mb-4 max-w-full inline-block">
          <div className="whitespace-pre-line">
            Update complete co-borrower information. All fields marked with * are required.
          </div>
        ...
        {/* Co-borrower Specific Section */}
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-gray-900 border-b pb-2">Co-Borrower Information</h3>
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="liveTogether" className="text-sm font-medium text-gray-700">
              Do you live at the same address as the primary borrower?
            </Label>
          </div>
        </div>
